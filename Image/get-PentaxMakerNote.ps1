...
function Get-PentaxExif {
  ...
  Process {
    ...
    if ($s -eq $null) {
        Write-Warning "No camera ID was found for $($image.fullname); giving up"; return
    }
    else {     $PentaxModel = @{"13" = "Optio 430";"76450"="*ist-D"; "76830"="K10D" ; "77240" = "K7"       }[$s]
    if   (-not $PentaxModel) {
               $PentaxModel = "Unknown"
    }}
    ...
    Elseif ([boolean][int]$s[3]) {
          $t = @{"1"="Multiple Exposure. ";  "16"="HDR. "; "32"="HDR Strong 1. ";  "48"="HDR Strong 2. ";
                "64"="HDR Strong 3. "     ; "224"="HDR Strong Auto. "}[($S[3])]
    }
    Else {
...
end {Write-Progress -Activity "Getting Pentax Exif Data for" -Completed -Status " "
  } 
}