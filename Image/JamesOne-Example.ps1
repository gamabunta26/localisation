...
$ReferenceImagePath = "E:\DCIM\100PENTX\IMG43272.JPG"
$offset             = $null 
$filesToTag         = $null 
$NMEAData           = $False
$CSVData            = $True
...
if ($refDate -and $ReferenceImagePath  ) {
        ...
        $picTime = (get-image $ReferenceImagePath | get-Exif).dateTaken
        $offset  = ( $picTime -  $refdate.ToUniversalTime() ).totalSeconds
}
       ...
       {   0 { $offset = 0 }
           1 { if (-not $refDate) {$RefDate = ([datetime]( Read-Host ("Please enter the Date & time, in the reference picture, formatted as" + [char]13 + [Char]10 +
                ...
                if ($ReferenceImagePath -and (test-path $ReferenceImagePath) -and $RefDate) { 
                   $picTime =  (get-image $ReferenceImagePath | get-Exif).dateTaken
                   $offset = ( $picTime - $refdate).totalSeconds
                }
